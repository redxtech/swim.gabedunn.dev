<template>
  <tr :class="{ 'bg-green-300': even }">
    ...
    <td
      class="font-bold"
      v-text="convert(event.time, 0)"
    />
    <td v-text="convert(event.time, 3)" />
    <td v-text="convert(event.time, 5)" />
    <td v-text="convert(event.time, 10)" />
    <td v-text="convert(event.time, 15)" />
    ...
    <td v-text="convert(usports[event.course][event.stroke][event.distance], 0)" />
    <td v-text="convert(usports[event.course][event.stroke][event.distance], 1.5)" />
    <td v-text="convert(usports[event.course][event.stroke][event.distance], 5)" />
    <td v-text="convert(usports[event.course][event.stroke][event.distance], 7.5)" />
  </tr>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      convert (time, percent) { return this.round(time * (1 + (percent / 100)), 2).toFixed(2) }
    }
  }
</script>
